import { OllamaServiceResponse, SummaryServiceRequest } from '@storyscribe-ai/model/types';
...
async function summarize(transcriptions: string[]): Promise<string[]> {
  ...
  for (let i = 0; i < transcriptions.length; i++) {
    ...
    try {
      const {
        data: { response },
      } = await axios.post<OllamaServiceResponse, AxiosResponse<OllamaServiceResponse>, SummaryServiceRequest>(
        '/summarize',
        {
